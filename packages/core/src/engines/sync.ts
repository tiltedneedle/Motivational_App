      ...
      rows: [
        {
          ...
          shift_days: p.shiftDays ?? [],
          shift_wake_time: p.shiftWakeTime ?? '13:00',
          shift_evening_time: p.shiftEveningTime ?? '23:00',
...
export function fromRows(tables: Partial<Record<(typeof TABLE_ORDER)[number], Row[]>>, defaults: Profile): SyncBundle {
  ...
  const profile: Profile = pr
    ? {
        ...
        // Absent before 0011: every day the same.
        shiftDays: Array.isArray(pr.shift_days) ? (pr.shift_days as unknown[]).map(Number).filter((d) => Number.isInteger(d) && d >= 0 && d <= 6) : defaults.shiftDays,
        shiftWakeTime: str(pr.shift_wake_time) || defaults.shiftWakeTime,
        shiftEveningTime: str(pr.shift_evening_time) || defaults.shiftEveningTime,
        ...
        writeWhen: (['morning', 'evening', 'any'] as const).find((w) => w === pr.write_when) ?? defaults.writeWhen,
