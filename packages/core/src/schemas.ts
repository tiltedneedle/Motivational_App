/**
 * The zod schemas behind the Morrow domain types (`types.ts`).
 *
 * Kept apart from the types on purpose: the app imports its types from
 * `types.ts`, which is type-only and carries no zod at runtime, so the
 * validator (half a megabyte of it) is not in the first load of a page that
 * validates nothing. Only what actually validates — a pull from the account
 * — imports from here.
 */
import { z } from 'zod';
