...
3. Authentication → Email. The app asks for a **six-digit code**, and `config.toml` carries the template for it (`supabase/templates/magic_link.html`, `{{ .Token }}`) — commented out, because a free-tier project on the default email provider refuses template changes. Until the project is on Pro (or has its own SMTP) the email carries Supabase's link, and the app signs in from the link too: tapped on the phone it opens `morrow://` with a one-time code, which the root layout exchanges for a session (the client uses PKCE, so the link has to be opened on the device that asked for it; the six-digit code is the path that works across devices). On Pro, uncomment the two template tables and `pnpm sb config push --yes`. Authentication → Providers → Apple: `config.toml` enables it with the bundle id `app.morrow.client` as the client id (native sign-in needs no secret).
4. The account, end to end, against the project: `pnpm build:web && pnpm test:account`. A throwaway user is made with the service role (from `supabase/.env.local`, never the browser), its session placed where a signed-in device keeps it, the seeded store pushed, the device wiped, the Book brought back and compared, the account closed through the deployed function, the user removed. Not part of `verify` — it reaches the network.
5. How the copy works (2026-09-22): the account holds one device's copy at a time, and the profile row says which device made it and when. A device pushes only over its own copy; if another device has copied since, the push stops and You offers "Bring that copy here" or "Replace it with this phone's". A first sign-in pulls the account's copy onto an empty phone (a two-minute first line is kept), pushes an empty account from a phone with writing, and asks when both hold writing — nothing moves until the person chooses. A phone wiped with Delete everything is a new device to the account (it asks again rather than pushing the fresh start over the Book). A closed account stays closed: no push reopens it (migration 0014's trigger refuses the un-delete from a user session), a sign-in inside the week is told and offered Reopen or Leave it closed, and an account that never copied anything is still swept. What travels: the profile and its settings, every goal, line, edition, plan, ledger row, letter, scene, portrait (migration 0015) and Present/Past entry — never a sitting the safety screen flagged, never a draft. On a phone the session is encrypted at rest (an AES key in the keychain, the ciphertext in app storage); in a browser it is the browser's own storage, and Safari clears a site's storage after a week without a visit — Today says so once and offers Add to Home Screen or a sign-in.
