...
import { Body, Chip, InkButton, Label, Notice, Statement, Studio, TextButton, TopBar, UserField, day } from '@morrow/ui';
...
export default function Present() {
  ...
  const [selected, setSelected] = useState<string[]>(() => mine.map((p) => p.cardId));
      ...
      <Studio testID="screen-present-write">
        <SafeAreaView style={{ flex: 1, paddingHorizontal: 22 }}>
          <TopBar
            back={{
              onPress: () => {
                // Back drops this card from the picks rather than losing the
                // writing silently: nothing half-written is ever stored.
                setSelected((cur) => cur.filter((x) => x !== writing.id));
                setStory('');
                setApply('');
              },
              testID: 'present-write-back',
            }}
